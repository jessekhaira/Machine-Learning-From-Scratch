...
class tests(unittest.TestCase):
    def testMultiClass(self):
        ...
        classificationObj2 = ClassificationTree(entropy=False, minSamplesSplit=1)
    ...
    def test_sanityChecks(self):
        classificationObj2 = ClassificationTree(entropy=False, minSamplesSplit=5, maxDepth=3, min_impurity_decrease=0.09)
        classificationObj2.fit(X1,Y1)
        predictions2 = classificationObj2.predict(X1)
        acc2 = accuracy(Y1, predictions2)

        classificationObj3 = ClassificationTree(entropy=False, minSamplesSplit=10, maxDepth=0, min_impurity_decrease=0.15)
        classificationObj3.fit(X1,Y1)
        predictions3 = classificationObj3.predict(X1)
        acc3 = accuracy(Y1, predictions3)

        classificationObj4 = ClassificationTree(entropy=False, minSamplesSplit=1000, maxDepth=10, min_impurity_decrease=0.15)
        classificationObj4.fit(X1,Y1)
        predictions4 = classificationObj4.predict(X1)
        acc4 = accuracy(Y1, predictions4)

        classificationObj5 = ClassificationTree(entropy=False, minSamplesSplit=10, maxDepth=10, min_impurity_decrease=1)
