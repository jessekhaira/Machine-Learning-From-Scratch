...
class Base_Regression(NeuralNetwork_Base):
    ...
    def __init__(self, degree, regularization = None, regParameter = None):
        ...
        super(Base_Regression, self).__init__(lossFunction = lossFunction, input_features = None)

    def fitGD(self, xtrain, ytrain, xvalid = None, yvalid = None, num_epochs =10, batch_size = 32, ret_train_loss = False, learn_rate = 0.01, optim = gradientDescent()):
        ...
        return self.fit(xtrain, ytrain, xvalid = xvalid, yvalid = yvalid, num_epochs = num_epochs, batch_size = batch_size, ret_train_loss = ret_train_loss, learn_rate = learn_rate, optim = optim)
    ...
    def _getCombos(self, numFeatures, degree):
        allCombos = [combinations_with_replacement(range(numFeatures),i) for i in range(1, degree+1)]
        unrolled = [item for sublist in allCombos for item in sublist]
        return unrolled

class LinearRegression(Base_Regression):
    def __init__(self, degree):
        super(LinearRegression, self).__init__(degree = degree, regularization= None, regParameter=None)

class RidgeRegression(Base_Regression):
    def __init__(self, degree, regParam = 0.2):
        super(RidgeRegression, self).__init__(degree = degree, regularization= "L2", regParameter=regParam)

class LassoRegression(Base_Regression):
    def __init__(self, degree, regParam = 0.2):
        super(LassoRegression, self).__init__(degree = degree, regularization= "L1", regParameter=regParam)
