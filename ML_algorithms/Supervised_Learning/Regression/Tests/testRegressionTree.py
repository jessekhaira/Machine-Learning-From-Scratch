...
class tests(unittest.TestCase):
    ...
    def test_sanityChecks(self):
        ...
        regressionObj2 = RegressionTree(minSamplesSplit=5, maxDepth=3, min_impurity_decrease=0.15)
        ...
        regressionObj3 = RegressionTree(minSamplesSplit=10, maxDepth=0, min_impurity_decrease=0.15)
        regressionObj3.fit(X1,Y1)
        predictions3 = regressionObj3.predict(X1)
        error3 = RMSE(Y1, predictions3)

        regressionObj4 = RegressionTree( minSamplesSplit=1000, maxDepth=10, min_impurity_decrease=0.15)
        regressionObj4.fit(X1,Y1)
        predictions4 = regressionObj4.predict(X1)
        error4 = RMSE(Y1, predictions4)

        regressionObj5 = RegressionTree( minSamplesSplit=10, maxDepth=10, min_impurity_decrease=1)
