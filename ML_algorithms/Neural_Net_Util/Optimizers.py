...
class gradientDescentMomentum(optimizer):
    ...
    def updateParams(self, params, dparams, learn_rate, epochNum = None):
        ...
        for i in range(len(params)):
            self.runningGradients[i] = self.beta*self.runningGradients[i] + (1-self.beta)*dparams[i]
...
class AdaGrad(optimizer):
    ...
    def updateParams(self, params, dparams, learn_rate, epochNum = None):
        ...
        for i in range(len(params)):
            ...
            params[i] = params[i] - (learn_rate*dparams[i])/(np.sqrt(self.runningGradients[i]+self.eps))
        return params 
...
class RMSProp(optimizer):
    ...
    def updateParams(self, params, dparams, learn_rate, epochNum = None):
        ...
        for i in range(len(params)):
            self.runningGradients[i] = self.beta*self.runningGradients[i] + (1-self.beta)*np.square(dparams[i])
            params[i] = params[i] - (learn_rate*dparams[i])/(np.sqrt(self.runningGradients[i]) + self.eps)
...
class Adam(optimizer):
    ...
    def updateParams(self, params, dparams, learn_rate, epochNum = None): 
        ...
        if not self.runningGradients:
            for i in range(len(params)):
                self.runningGradients.append([np.zeros_like(params[i]), np.zeros_like(params[i])])
        ...
        for i in range(len(params)):
            ...
            adaptiveLR = self.beta2*adaptiveLR + (1-self.beta2)*np.power(dLdparam,2)
            ...
            params[i] = params[i] - (learn_rate*momentum)/(np.sqrt(adaptiveLR)+self.eps)
