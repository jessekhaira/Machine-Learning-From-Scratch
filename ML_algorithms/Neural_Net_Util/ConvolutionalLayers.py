...
from misc import findRowColMaxElem
...
class convBase(_BaseLayer):
    def _padInput(self, x, filterSize, padding):
        ...
        images_padded = np.pad(x, ((0, 0),(0,0), pad_h, pad_w), mode='constant', constant_values = 0)
...
class Conv2D(convBase):
    ...
    """
    def __init__(self, filterSize, numFilters, activationFunction, padding = "same", stride=1, finalConvLayer = False, inputDepth = None):
    ...
    def compute_forward(self, x, train=True):
        ...
        outputHeight = int((x.shape[2]-self.filterSize+2*p)/(self.stride) + 1)
        outputWidth = int((x.shape[3] - self.filterSize+2*p)/(self.stride) + 1)
        self.Z = np.zeros((x.shape[0], self.numFilters, outputHeight, outputWidth))
            ...
            for filter in range(self.numFilters):
                ...
                while curr_rowPic + self.filterSize <= image.shape[1]:
                    ...
                    while curr_colPic + self.filterSize <= image.shape[2]:
                        curr_imageSlice = image[:, curr_rowPic:curr_rowPic+self.filterSize, curr_colPic:curr_colPic+self.filterSize]
                        # for this image and this filter, we fill curr_rowNeuron and curr_colNeuron with the value shown 
                        self.Z[i, filter, curr_rowNeuron, curr_colNeuron] = np.sum(curr_imageSlice*filterWeights)+bias 
    ...
    def _initializeWeights(self):
        ...
        filters = np.random.rand(self.numFilters, self.inputDepth, self.filterSize, self.filterSize)*0.01
        # we have one bias term for each filter 
        bias = np.zeros((self.numFilters,1))
        return filters, bias

    def _updateWeights(self, dLdA, learn_rate, epoch, prediction_obj, curr_x, curr_y, layer, gradCheck = False):
        ...
        if self.finalConvLayer:
            dLdA = dLdA.reshape(self.Z.shape[0], self.Z.shape[1], self.Z.shape[2], self.Z.shape[3])
            ...
            for filter in range(self.numFilters):
                ...
                while curr_rowPic + self.filterSize <= image.shape[1]:
                    ...
                    while curr_colPic + self.filterSize <= image.shape[2]:
                        ...
                        # filters see 
                        curr_imageSlice = image[:, curr_rowPic:curr_rowPic+self.filterSize, curr_colPic:curr_colPic+self.filterSize]
                        neuron_gradientHere = dLdZ[i, filter, curr_rowNeuron, curr_colNeuron] 
                        ...
                        dLdA_prevLayer[i, :, curr_rowPic:curr_rowPic+self.filterSize, curr_colPic:curr_colPic+self.filterSize] += (self.filters[filter]*neuron_gradientHere)
        ...
        self.filters, self.b = self.optim.updateParams([self.filters, self.b], [dLdW, dLdb], learn_rate, epochNum = epoch + 1)
...
class Pool(convBase):
    ...
    """
    def __init__(self, filterSize, padding = "valid", stride=1, poolType = "max", finalConvLayer = False):
        self.filterSize = filterSize
    ...
    def compute_forward(self, x, train=True):
        ...
        outputHeight = int((x.shape[2]-self.filterSize+2*p)/(self.stride) + 1)
        outputWidth = int((x.shape[3] - self.filterSize+2*p)/(self.stride) + 1)
        self.Z = np.zeros((x.shape[0], x.shape[1], outputWidth, outputHeight))
        ...
        for i in range(padded_input.shape[0]):
            ...
            for dimension in range(image.shape[0]):
                ...
                while curr_rowPic + self.filterSize <= image.shape[1]:
                    ...
                    curr_colNeuron = 0 
                    while curr_colPic + self.filterSize <= image.shape[2]:
                        curr_imageSlice = image[dimension, curr_rowPic:curr_rowPic+self.filterSize, curr_colPic:curr_colPic+self.filterSize]
                        # max pool or average pool 
                        if self.typePool == "max":
                            self.Z[i, dimension, curr_rowNeuron, curr_colNeuron] = np.max(curr_imageSlice)
                        else:
                            self.Z[i, dimension, curr_rowNeuron, curr_colNeuron] = np.average(curr_imageSlice)
    ...
    def _updateWeights(self, dLdA, learn_rate, epoch, prediction_obj, curr_x, curr_y, layer, gradCheck = False):
        ...
        dLdA_in = np.zeros((self.Ain.shape[0], self.Ain.shape[1], self.Ain.shape[2], self.Ain.shape[3]))
            ...
            for dimension in range(image.shape[0]):
                ...
                while curr_rowPic + self.filterSize <= image.shape[1]:
                    ...
                    while curr_colPic + self.filterSize <= image.shape[2]:
                        ...
                        if self.typePool == "max":
                            maxIdx_row, maxIdx_col = findRowColMaxElem(curr_imageSlice)
                            dLdA_in[i, dimension, maxIdx_row, maxIdx_col] += 1 * dLdA[i, dimension, maxIdx_row, maxIdx_col] 
                        ...
                        # since all the neurons in the img slice just get averaged 
                            ...
                            dLdA_in[i, dimension, curr_rowPic:curr_rowPic+self.filterSize, curr_colPic:curr_colPic+self.filterSize] += (1/np.size(curr_imageSlice) * dLdA[i, dimension, curr_rowNeuron, curr_colNeuron])
