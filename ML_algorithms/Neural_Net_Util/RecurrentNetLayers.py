...
class RNN_cell_languageModel(_BaseLayer):
    ...
    def _train_forward(self, x, y, a_prev, charToIdx, cache = True):
        ...
        for t in range(len(x)):
            ...
            A, predictions, a_prev, x_t, Z_activ = self._compute_OneStepForward(x_t, a_prev)
            ...
            if cache: 
                self.cache_perTimestep[t] = (A, predictions, a_prev, x_t, Z_activ) 
    ...
    def _updateWeights(self, learn_rate, total_timeSteps, y_label, charToIdx, epochNum, optim):
        """
        ...
        dLdBa, dLdWaa, dLdWax = np.zeros_like(self.ba), np.zeros_like(self.Waa), np.zeros_like(self.Wax)
        ...
        for t in reversed(range(total_timeSteps)):
            ...
            dadZ_activ= self.activationFunctionLayer.getDerivative_wrtInput(Z_activ)
        ...
        self.Way, self.Waa, self.Wax, self.ba, self.by = optim.updateParams(params, dparams, learn_rate, epochNum)


    def generate(self, seedVector, a_prev, totalGeneratingSteps, idxToChar, temperature):
        """
        ...
        for i in range(totalGeneratingSteps):
            A, predicted_vector, _, _, _ = self._compute_OneStepForward(x_t, a_prev, temperature)
            # lets choose a letter weighted by the probability given by the predicted vector
            predicted_index = np.random.choice(self.numInputFeatures, p = predicted_vector.ravel())
