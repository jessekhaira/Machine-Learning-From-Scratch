...
from ML_algorithms.Unsupervised_Learning.k_Means import k_Means
...
class tests(unittest.TestCase):
    def testIris(self):
        ...
        for i, m, color in zip(range(3), markers, colors):
            ...
            scatterI = ax.scatter(X[0,ithClusterPts], X[1, ithClusterPts], X[3, ithClusterPts], c = color, marker = m)
            scatter.append(scatterI)
        ax.legend(scatter, ('cluster1','cluster2','cluster3',))
        ax.set_xlabel('Dim1')
        ax.set_ylabel('Dim2')
        ax.set_zlabel('Dim3')
        plt.show() 
