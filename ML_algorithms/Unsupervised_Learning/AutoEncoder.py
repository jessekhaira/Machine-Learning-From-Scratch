import numpy as np 
from ML_algorithms.Neural_Net_Util.NeuralNet_Layers import DenseLayer, BatchNormLayer_Dense
...
class Deep_Autoencoder(NeuralNetwork_Base):
    ...
    def _buildEncoder(self):
        encoder = [] 
        encoder.append(BatchNormLayer_Dense(num_in = 784, num_layer=300, activationFunction = ReLU()))
        encoder.append(BatchNormLayer_Dense(num_in =300, num_layer=150, activationFunction = ReLU()))
        # fully encoded units
        encoder.append(BatchNormLayer_Dense(num_in = 150, num_layer = self.size_encoding, activationFunction = ReLU()))
        return encoder 

    def _buildDecoder(self):
        decoder = [] 
        decoder.append(BatchNormLayer_Dense(num_in= self.size_encoding, num_layer = 150, activationFunction = ReLU()))
        decoder.append(BatchNormLayer_Dense(num_in = 150, num_layer =300, activationFunction= ReLU()))
        ...
        decoder.append(DenseLayer(num_in =300, num_layer=self.num_features, activationFunction= Sigmoid()))
        return decoder

    def fit(self, xtrain, num_epochs =10, batch_size = 32, ret_train_loss = False, learn_rate = 0.1, optim = gradientDescent(), verbose = False):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(num_batches):
                ...
                backpropInit = self.lossFunction.derivativeLoss_wrtPrediction(curr_x, pred_miniBatch)
                self._backward_propagate(backpropInit, learn_rate, optim, epoch, curr_x, curr_x)
    ...
    def imgs_checkpoints(self, epoch, X):
        ...
        # get generated imgs
        imgs = self.predict(train_9, supervised=False).reshape(-1,self.img_height, self.img_width)
        imgIdx = 0 
